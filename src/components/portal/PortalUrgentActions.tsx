...
const PortalUrgentActions: React.FC<PortalUrgentActionsProps> = ({ financialAssets }) => {
  // Build urgent action items from financial assets that have contact info
  ...
  for (const asset of financialAssets) {
    ...
    if (asset.category === 'insurance') {
      ...
      actions.push({
        label: `Contact ${asset.institution || asset.name} about life insurance policy${policyNum}${refSuffix}`,
      ...
      actions.push({
        label: `Contact ${asset.institution || asset.name} about account ending ${last4}`,
        phone: asset.contact_phone,
        email: asset.contact_email,
      });
      ...
      actions.push({
        label: `Review mortgage with ${csf.mortgage_provider}${balance}`,
        phone: asset.contact_phone,
        email: asset.contact_email,
      });
      ...
      actions.push({
        label: `Contact ${asset.institution || asset.name} about outstanding debt${balance}`,
        phone: asset.contact_phone,
        email: asset.contact_email,
      });
    } else {
      actions.push({
        label: `Contact ${asset.institution || asset.name} regarding ${asset.name}`,
        phone: asset.contact_phone,
        email: asset.contact_email,
      });
    }
  ...
  return (
    <div className="bg-warning/10 backdrop-blur-xl rounded-2xl p-6 border border-warning/30">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-base font-semibold text-white">Urgent Actions</h3>
      </div>
      <div className="space-y-3">
        {actions.map((action, i) => (
          <div key={i} className="flex items-start gap-3 bg-white/5 rounded-xl p-3">
            <CheckSquare className="w-4 h-4 text-warning mt-0.5 shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-white/90 text-sm">{action.label}</p>
              <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                {action.phone && (
                  <a href={`tel:${action.phone}`} className="inline-flex items-center gap-1 text-primary text-xs hover:underline">
                    <Phone className="w-3 h-3" />{action.phone}
                  </a>
                )}
                {action.email && (
                  <a href={`mailto:${action.email}`} className="inline-flex items-center gap-1 text-primary text-xs hover:underline">
