...
function buildGracePeriodHtml(userName: string, intro: string, gracePeriodHours: string): string {
  ...
  const safeHours = escapeHtml(gracePeriodHours);
  return `<!DOCTYPE html><html><body style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;"><div style="background: linear-gradient(135deg, #f59e0b, #d97706); color: white; padding: 32px; text-align: center; border-radius: 12px 12px 0 0;"><h1 style="margin:0; font-size:24px;">⚠️ Grace Period Started</h1><p style="margin:12px 0 0; opacity:0.9; font-size:14px;">Dead Man's Switch Warning</p></div><div style="background: white; padding: 32px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;"><p>Hello <strong>${safeName}</strong>,</p><p style="color:#4b5563;">${safeIntro} A <strong>${safeHours}-hour grace period</strong> has now started.</p><div style="background:#fee2e2; border:2px solid #ef4444; padding:20px; border-radius:8px; text-align:center; margin:24px 0;"><p style="color:#991b1b; font-weight:600;">⏰ GRACE PERIOD ENDS:</p><p style="color:#dc2626; font-size:18px; font-weight:700;">${escapeHtml(formatDateTime(new Date(Date.now() + parseInt(gracePeriodHours) * 60 * 60 * 1000).toISOString()))}</p></div><hr style="border:none; border-top:1px solid #e5e7eb; margin:32px 0;"><p style="font-size:13px; color:#9ca3af; text-align:center;">This email was sent by your Dead Man's Switch system.</p></div></body></html>`;
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (templateType === "switch_triggered") {
      ...
      html = `<!DOCTYPE html><html><body style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;"><div style="background: linear-gradient(135deg, #dc2626, #991b1b); color: white; padding: 32px; text-align: center; border-radius: 12px 12px 0 0;"><h1 style="margin:0; font-size:24px;">${escapeHtml(headerTitle)}</h1><p style="margin:12px 0 0; opacity:0.9; font-size:14px;">${escapeHtml(headerSubtitle)}</p></div><div style="background: white; padding: 32px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;"><p>Dear <strong>Test Contact</strong>,</p><p style="color:#4b5563;">${escapeHtml(introMessage)}</p><div style="background:#f3f4f6; padding:24px; text-align:center; border-radius:8px; margin:20px 0;"><p style="color:#6b7280;">📄 Your shared documents and messages would appear here.</p></div><hr style="border:none; border-top:1px solid #e5e7eb; margin:32px 0;"><p style="font-size:13px; color:#9ca3af; text-align:center;">${escapeHtml(footerMessage)}</p></div></body></html>`;
    }
