...
function filterFinancialAssets(assets: any[], contactId: string): any[] {
  // Opt-in: no visible_to (null or []) means NOT shared with anyone (H3).
...
async function servePortalResponse(
  ...
  // Trigger gate (C2): never return vault data until the owner's switch has fired.
  const { data: settings } = await supabase
    .from("user_settings")
    .select("switch_triggered, switch_triggered_at")
  ...
  if (!settings?.switch_triggered) {
    return new Response(
      JSON.stringify({
        released: false,
        error: "This vault has not been released yet. The owner is still active.",
      }),
  ...
  try {
    ...
    const { data: share } = await supabase
      .from("contact_shares")
      .select("encrypted_content, content_iv, kdf_salt, kdf_iterations, needs_regeneration")
      .eq("access_token_hash", tokenHash)
      .eq("contact_id", tokenData.contact_id)
      .maybeSingle();

    if (share?.needs_regeneration) {
      return new Response(
        JSON.stringify({
          error: "This portal link needs to be regenerated by the vault owner. Please contact them.",
        }),
    ...
    if (share?.encrypted_content && share?.content_iv) {
      return new Response(
        JSON.stringify({
          ...
          kdfSalt: share.kdf_salt || null,
          kdfIterations: share.kdf_iterations || null,
